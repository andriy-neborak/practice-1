...
class Match3Game:
    def __init__(self):
        pygame.init()
        pygame.mixer.init()  # Ініціалізація аудіо системи
        ...
        # --- ЗАВАНТАЖЕННЯ ЗВУКІВ ---
        # Завантажуємо звук 3-в-ряд ("I love you")
        try:
            self.sound_match3 = pygame.mixer.Sound(
                "yoursperfectguy-i-love-you-cute-message-notification-sound-sfx-1-334729.mp3")
            self.sound_match3.set_volume(0.9)
        except Exception as e:
            print("Не знайдено звук 3-в-ряд:", e)
            self.sound_match3 = None

        # Завантажуємо звук 4-в-ряд ("Among Us")
        try:
            self.sound_match4 = pygame.mixer.Sound("among.mp3")
            self.sound_match4.set_volume(0.9)
        except Exception as e:
            print("Не знайдено звук 4-в-ряд:", e)
            self.sound_match4 = None
        ...
        with self.lock:
            while self.queue:
                ...
                if cmd == 0x16:
                    ...
                    new_score = (p[1] << 24) | (p[2] << 16) | (p[3] << 8) | p[4]

                    # --- ЛОГІКА ЗВУКІВ ЗАЛЕЖНО ВІД ПРИРОСТУ БАЛІВ ---
                    if new_score > self.score:
                        score_diff = new_score - self.score

                        # Якщо додалося 60 або більше балів (це мінімум 4 кульки або комбо)
                        if score_diff >= 60:
                            if self.sound_match4: self.sound_match4.play()
                        # Якщо додалося 30 балів (це рівно 3 кульки)
                        elif score_diff >= 30:
                            if self.sound_match3: self.sound_match3.play()
    ...
    def is_board_stable(self):
        if time.time() - self.last_board_update_time < 0.4:
            return False
    ...
    def click(self, pos):
        ...
        if self.state == "MENU":
            ...
            if self.busy:
                if time.time() - self.busy_start_time > 1.5:
                    self.busy = False
                else:
                    self.selected = None
                    return

            if not self.connected or not self.is_board_stable():
                self.selected = None
                return
    ...
    def draw(self):
        ...
        if self.state == "MENU":
            ...
            name_txt = self.font_big.render(display_name + ("_" if time.time() % 1 > 0.5 else ""), True,
                                            (255, 255, 255))
            ...
            menu_btn_rect = pygame.Rect(WIDTH - 120, 10, 100, 40)
            self.draw_button(self.screen, menu_btn_rect, "MENU", (180, 60, 60), self.font_small,
                             menu_btn_rect.collidepoint(mx, my))
        ...
        self.draw_button(self.screen, self.rect_right, ">", ACCENT, self.font_small,
        ...
        self.draw_button(self.screen, self.rect_conn, btn_text, btn_color, self.font_small,
                         self.rect_conn.collidepoint(mx, my))
